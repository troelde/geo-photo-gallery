...
- **Export PDF tab**: click **Generate & download PDF** to build a single PDF of the whole gallery entirely in your browser (no server involved) — a title page with the gallery description, one section per day (with that day's description, if any), and one page per photo showing its image (medium/large OneDrive thumbnail), filename, effective caption, taken date, and GPS position with a "View on map" link. A status line shows progress ("Rendering photo N of M…") while it builds; if an individual photo's thumbnail can't be fetched, that photo's page still renders with its caption/metadata and an "(image unavailable)" note instead of failing the whole export.
