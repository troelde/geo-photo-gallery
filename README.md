...
- **Export PDF tab**: click **Generate & download PDF** to build a single PDF of the whole gallery entirely in your browser (no server involved) — a title page with the gallery description, one section per day (with that day's description, if any), and its photos laid out **4 per page** in a 2×2 grid, each with its image (medium/large OneDrive thumbnail), filename, effective caption, taken date, and GPS position with a "View on map" link (fields are truncated to fit the compact grid cell). A status line shows progress ("Rendering photo N of M…") while it builds; if an individual photo's thumbnail can't be fetched, that photo's cell still renders its caption/metadata and an "(image unavailable)" note instead of failing the whole export.
- The gallery description and each day's description are rendered **richly** in the PDF, not just as plain text: headings, **bold**/*italic*, and bullet lists are preserved, Markdown links (e.g. `[View on map](https://...)`) become real clickable links in the PDF, and any Markdown image (e.g. `![Map](https://...)`) is fetched and embedded directly into the page. If an image in a description can't be fetched/decoded, the export still completes — it just prints an "[image unavailable: ...]" note in its place instead of failing.
